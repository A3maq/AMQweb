...
define("tmip_lang_update_settings", 'Update TraceMyIP Plugin Settings');
...
function tmip_reports_page() {
	...
	echo '
		...
		<p>Your browser does not support iframes, which is required by '.service_name.' in order to display user interface for the web tracking data within your WordPress dashboard.</p>
		</iframe>
	';
}
...
function tmip_options_page() {
	...
	$output .='<h2>About '.tmip_service_Dname.'</h2>
		...
		<ol class="tmip_sett_list_ol">
			...
			<li><span>Visitor redirecting and blocking based on custom rules by an IP address, location, hardware specifications, etc.</span></li>
	...
	$output .='
		...
		<ol tmip_sett_list_ul>
			...
			<li><span>If you would like to control visitor access to your pages and/or send one-way messages to specific visitors, first, ensure that the Visitor Tracker code is installed properly and working. Next, go to the Page Tracker menu option under your project, generate a  '.tmip_lang_page_tr_code.' and place it into the '.tmip_lang_page_tr_code.' input box below.</span></li>
		</ol>
		</section>
		</blockquote>';
